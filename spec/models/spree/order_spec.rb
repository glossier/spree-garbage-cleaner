...
describe Spree::Order do
  ...
  context 'class methods' do
    ...
    context '.destroy_garbage' do
      it 'has a method to destroy garbage' do
        Spree::Order.destroy_garbage.should eq(
          destroyed: [@order_one, @order_two],
          errors: []
        )
        Spree::Order.garbage.count.should eq(0)
      ...
      it 'destroys garbage in batches' do
        ...
        dummy_garbage.should_receive(:find_each)
                     .with(batch_size: Spree::GarbageCleaner::Config.batch_size)
  ...
  context 'associated objects' do
    let(:order) do
      create(:order, created_at: (ordered_on + rand(10)).days.ago, completed_at: nil)
    end
